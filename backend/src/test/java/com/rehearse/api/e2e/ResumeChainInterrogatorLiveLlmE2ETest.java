 ...
 * </pre>
 *
 * 비용 발생 + 비결정적 → CI 미실행 (@Disabled). best_answer 품질 회귀 의심 시 수동 실행.
 */
...
class ResumeChainInterrogatorLiveLlmE2ETest extends AbstractMySqlContainerTest {
    ...
    @Test
    @DisplayName("build 가 L4 chain topic + 레벨 깊이에 맞는 best_answer 를 채운다 + AC1/AC4/AC5 단언")
    void build_returns_level_appropriate_best_answer_for_l4() {
        ...
        assertThat(result.bestAnswer())
                .as("AC1 best_answer — 1인칭 답변 예시 어미 1+")
                .containsAnyOf("했습니다", "경험이 있습니다", "을 적용했습니다", "을 진행했습니다", "을 측정했습니다");
        assertThat(result.bestAnswer())
                .as("AC1 best_answer — 가이드 톤 어구 부재")
                .doesNotContain("해보세요", "하면 좋습니다", "이야기해보세요", "설명해보세요");
        assertThat(result.bestAnswer())
                .as("AC4 best_answer — chain topic 명시")
                .contains(chainTopic);
        assertThat(result.bestAnswer())
                .as("AC4 best_answer — L4 어휘 1+ (트레이드오프 / 비교 / 측정 / 장단점 / 대안 / 선택)")
