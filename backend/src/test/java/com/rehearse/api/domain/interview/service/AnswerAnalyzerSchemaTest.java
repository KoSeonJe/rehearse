...
@DisplayName("AnswerAnalyzer — strict JSON Schema 정의 검증")
class AnswerAnalyzerSchemaTest {

    @Test
    @DisplayName("buildJsonSchema() root 는 5개 필드 모두 required + additionalProperties=false")
    void buildJsonSchema_root_requiresAllTopLevelFields() {
        @SuppressWarnings("unchecked")
        Map<String, Object> schema = (Map<String, Object>) AnswerAnalyzer.buildJsonSchema();
    ...
    void buildJsonSchema_dimensionGaps_requiresAll10Keys_asNullableInteger() {
        ...
        assertThat(dimensionGaps.get("required"))
                .isEqualTo(List.copyOf(AnswerAnalyzer.DIMENSION_KEYS));

        @SuppressWarnings("unchecked")
        Map<String, Object> dimensionGapsProps = (Map<String, Object>) dimensionGaps.get("properties");
        for (String key : AnswerAnalyzer.DIMENSION_KEYS) {
            @SuppressWarnings("unchecked")
