---
name: implement-be
description: "Backend 구현 진입 정형화 스킬. product-spec + tech-spec + implement.md 검증 → git worktree 생성 → spec 3종 1커밋 → backend agent 위임까지 자동. 메인 세션이 spec 누락한 채 구현 진입하는 사고 방지. '/implement-be {slug}', 'BE 구현 시작', 'backend implement 진입', '이 plan 백엔드 구현' 등 트리거. slug 인자 필수. 호출 시 docs/plans/{slug}/ 안의 spec 3종 검증 후 ../devlens-{slug} worktree + feat/{slug} 브랜치 생성. 코드 변경은 backend agent 가 implement.md 따라 자율 진행."
---

# Implement BE

Backend 구현 진입 정형 절차. spec 3종 검증 → worktree → spec 커밋 → `backend` agent 위임.
...
3. spec 3종 존재 확인 — 다음 패턴 중 **각 카테고리 하나씩** 충족:
   ...
   - implement (BE): `implement.md` 또는 `implement-be.md` 또는 `tasks/be-*.md`
...
이미 develop 에 머지돼 변경 0 = skip + "spec 이미 머지됨" 보고.

## Backend Agent 위임

`Read` 로 다음 파일 본문 흡수:
- `docs/plans/{slug}/implement.md` (또는 `implement-be.md`)
- `docs/plans/{slug}/tech-spec.md` (또는 `tech_spec/design.md`)
- `docs/plans/{slug}/product-spec.md` (또는 `product_spec/requirements.md`)
- `tasks/be-*.md` 존재 시 모두 (분리 임계 초과 케이스)

`Agent(subagent_type=backend, prompt=...)` 1회 호출. prompt 본문에 다음 인라인 포함:
...
1. `backend/.claude/rules/conventions.md`
2. `backend/.claude/rules/testing.md`
3. `backend/AGENTS.md`
4. `.claude/rules/commit.md` (커밋 메시지 룰)
5. `.claude/rules/security.md` (OWASP Top 10 self-check)
...
{implement.md 또는 implement-be.md 본문 그대로 + tasks/be-*.md 있으면 함께}

## 진행 방식

- implement plan 의 Phase 순서대로 끝까지 자율 진행. 사용자 사전 승인 = 본 스킬 호출.
- 각 Phase 완료마다 별도 커밋 (commit.md 룰: `feat(BE):`, `fix(BE):`, `refactor(BE):`, `test(BE):` 등).
- 빌드 / 테스트 통과 상태로만 커밋.
...
- 브랜치: feat/{slug}
- spec 커밋: {SHA 또는 "이미 머지됨"}
- backend agent 결과: {요약}

**다음 단계** (사용자 결정):
1. 통합 리뷰 — `code-reviewer-backend` 호출
...
- `.claude/rules/branch-pr.md`: `feat/{slug}` 네이밍 / 직접 push 금지 / PR 머지 사용자 승인 필요
...
- backend agent prompt 에 컨벤션 로드 지시 누락
- 보안 위반 / 모호함 발견했는데 agent 가 자율 진행
