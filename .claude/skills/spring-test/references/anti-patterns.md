...
class InterviewServiceTest {
    @MockitoBean InterviewRepository repo;
}

// GOOD
@ExtendWith(MockitoExtension.class)
class InterviewServiceTest {
    @Mock InterviewRepository repo;
...
**문제**: 유지보수 비용 증가.
**수정**: 헬퍼 메서드 추출, `@BeforeEach`에 공통 설정.
...
**증상**: `testCreateInterview()`.
**문제**: JUnit 4 잔재, `@Test`가 이미 표시.
**수정**: `createInterview_success()` 패턴으로 변경.
