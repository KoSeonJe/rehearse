...
**diff 라인만 보고 판단 금지**. 변경 라인은 빙산 일각 — 전체 구조 / 호출 경로 / 도메인 흐름 맥락에서 평가해야 결함·확장성 문제 보임.

리뷰 착수 전 **반드시** 다음 순서로 시야 확보:

1. **변경 파일 전체 Read** — diff hunk 외 파일 시작~끝. 메서드 길이 / 클래스 책임 / 다른 메서드와 일관성 / 기존 패턴 확인.
2. **호출 경로 추적** — 변경된 public 메서드 호출자 (`Grep`) / 의존 service / repository / event listener. 변경 영향 범위 사이드이펙트 후보.
3. **도메인 패키지 구조** — `domain/{name}/` 하위 entity / service / dto / repository 관계 파악. 책임 분리 / 결합도 / Rich Domain 여부 판단 근거.
4. **유사 / 인접 코드 패턴** — 같은 도메인 다른 service / 다른 도메인 같은 케이스 구현. 컨벤션 일탈 / 중복 / 추상화 기회 식별.
5. **테스트 / Flyway / 설정 동반 변경** — 코드만 보면 놓침. test fixture / V*.sql / application-*.yml / build.gradle.kts 변경 같이 검토.

이 단계 스킵 시 보이는 결함 = "변경 라인 자체 버그" 만. 보이지 않는 결함 = N+1 / 트랜잭션 정합성 / 호출자 회귀 / 도메인 결합 폭증. **시야 부족 리뷰 = P0 누락 위험**.
