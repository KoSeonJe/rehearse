...
| 비언어 분석 | 없음 또는 서버 AI 추론 (유료) | **브라우저에서 시선·표정·자세 분석 (서버 비용 $0)** |
...
AI 면접관이 음성으로 질문하고, 사용자의 답변을 실시간 인식합니다. 답변 내용에 따라 심화·보충·반론 후속 질문을 즉석에서 생성합니다.

면접 중 브라우저가 자동으로 시선·표정·자세를 추적하고, 음성 톤과 발화 패턴을 분석하여 타임스탬프 이벤트로 기록합니다.

### 3. 타임스탬프 피드백 리뷰

<!-- ![피드백 리뷰](docs/images/review.png) -->

면접 종료 후 AI가 답변 내용과 비언어 이벤트를 통합 분석하여 타임스탬프별 피드백을 생성합니다.
