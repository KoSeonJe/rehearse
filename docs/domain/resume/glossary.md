...
| 모드 / FSM 모드 | `ResumeMode` enum | resume 트랙 진행 단계. `PLAYGROUND` → `INTERROGATION` (단방향) | `api/process-user-turn.md#상태 전이` |
...
| Hard Timeout | `hard-timeout-min` (yml) | `elapsed ≥ duration + 10분` 시 강제 종료 (backstop) | `ResumeModeTransitionPolicy.isHardTimeoutExceeded` |
| FE 종료 신호 | `FollowUpRequest.terminate` | FE 시계 잔여 ≤ 0 + 답변 완료 제출 시점에 동봉. BE 는 답변 분석만 수행하고 신규 질문 INSERT skip → `followUpExhausted=true` 응답 | `ResumeInterviewOrchestrator.processUserTurn` |
...
| Fallback (폴백) — questions | `ResumeFallbackQuestions` | 3종 한국어 폴백 문장 (OPENER / PLAYGROUND_RESPONDER / INTERROGATION). LLM 응답이 동일 시 WARN 로그 (LLM-side 폴백 감지 패턴) | Issue #408 A3 |
