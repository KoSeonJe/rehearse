...
rehearse:
  features:
    lambda-nonverbal-schema-v2:
      enabled: false       # Lambda 배포 후 스모크 통과 시 true
      default-on-missing:  # 필드 누락 시 안전값
        speed_variance: 0.5
        gaze_on_camera_ratio: 0.5
        posture_unstable_count: 0
```

Lambda 는 Backend feature flag 와 독립이므로 실제 적용은 Lambda 환경변수 `ANALYZER_SCHEMA_VERSION=v2` 로 제어. Backend `feature flag` 는 결과 소비 스위치.
...
- 없음 (프롬프트 필드 확장은 영구 변경). 단, `lambda-nonverbal-schema-v2` feature flag 는 Backend 측에서 plan-12 시점에 제거 가능
