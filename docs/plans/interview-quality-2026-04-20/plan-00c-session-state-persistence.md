...
| `backend/src/main/java/com/rehearse/api/domain/interview/runtime/InterviewRuntimeState.java` | 신규. 턴 루프 워킹 메모리 (coveredClaims, activeChain, turnAnalysisCache) — LLM 재호출 비용 회피가 일차 목적 |
| `backend/src/main/java/com/rehearse/api/domain/interview/runtime/InterviewRuntimeStateStore.java` | 신규. Caffeine 기반 in-memory store (interviewId → RuntimeState, 세션 상한 기반 8h write TTL) |
...
| **L3. 턴 워킹 메모리** | `coveredClaims`, `activeChain`, `currentLevel`, `playgroundTurns`, `turnAnalysisCache` | **Caffeine in-memory (RuntimeStateStore)** | LLM 재호출 비용 회피가 일차 목적. SoT 필드 포함 → 세션 중 축출 금지 | write 기준 8h (세션 상한) |
...
별도 락 서비스를 두지 않는다. `InterviewRuntimeStateStore.update()` 가 Caffeine 의 `asMap().compute()` 를 호출하면 내부 `ConcurrentHashMap` 이 동일 key 에 대한 매핑 함수를 직렬화 실행한다. 다른 interviewId 는 독립 병렬:
...
**중요 제약**: mutator 안에서는 LLM/DB IO 금지. `asMap().compute()` 의 매핑 함수는 같은 key 의 다른 compute 를 블록한다 — 장시간 작업이 들어가면 전체 경로가 멈춘다. LLM 호출·DB 쓰기는 compute 밖에서 수행한 뒤 결과만 state 에 주입.

동일 interviewId 에 대한 중복 요청(이중 클릭 등) 방어는 컨트롤러 레벨 idempotency 로 막는다. Store 직렬화는 in-memory 상태 일관성만 보장하며 중복 턴 생성을 막지 않는다.
...
3. `InterviewRuntimeStateStore` 동시성 테스트(JUnit + 100 스레드) — 동일 interviewId 업데이트 race 없음 (`asMap().compute()` 직렬화 검증)
4. Caffeine 메트릭(`rehearse.runtime.state.cache.{hits,misses,evictions}`)이 Actuator로 노출
