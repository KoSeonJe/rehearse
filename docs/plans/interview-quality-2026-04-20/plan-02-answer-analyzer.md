...
| `backend/src/main/java/com/rehearse/api/domain/interview/AnswerAnalysis.java` | 신규 record. claims/missing_perspectives/unstated_assumptions/answer_quality. **`implements TurnAnalysis` (마커 인터페이스, `domain/interview/entity/TurnAnalysis.java` 이미 존재)** — plan-08 Rubric Scorer 와 캐시 키 통합 |
...
| `backend/src/main/java/com/rehearse/api/domain/interview/entity/InterviewRuntimeState.java` | **수정 (plan-00c에서 생성, 실제 위치는 `entity/`)**. `turnAnalysisCache` 필드는 이미 존재. 본 PR 에서 접근 메서드 신규 추가: `void recordAnalysis(Long turnId, TurnAnalysis analysis)` + `Optional<AnswerAnalysis> getAnswerAnalysis(Long turnId)` (instanceof 안전 캐스팅). `InterviewSession` 클래스는 실재하지 않음 — runtime 상태는 `InterviewRuntimeStateStore`로 관리 |
| `backend/src/main/resources/application.yml` | 수정. `rehearse.answer-analyzer.enabled: true` (기본 활성, Feature Flag 없음) |
| ~~`backend/src/main/resources/db/migration/V{XX}__alter_rubric_score_turn_fk.sql`~~ | **plan-08 로 이관 (2026-04-26)**. plan-02 PR 시점 `interview_turn` 테이블 부재 → FK 부착 불가. plan-08 전제 섹션에 흡수 (`docs/plans/interview-quality-2026-04-20/plan-08-rubric-scorer.md#전제`) |
...
`InterviewRuntimeState` 의 `turnAnalysisCache` 는 plan-00c 에서 `Map<Long, TurnAnalysis>` (`ConcurrentHashMap`) 로 이미 선언됨. 본 PR 에서는 두 메서드만 추가:
- `recordAnalysis(Long turnId, TurnAnalysis analysis)` — `turnAnalysisCache.put(turnId, analysis)` 위임
- `getAnswerAnalysis(Long turnId)` — `instanceof AnswerAnalysis` 안전 캐스팅 후 `Optional` 래핑 (plan-08 Rubric 결과와 혼선 방지)

### L1 False Negative 안전망 (critic M3 재정의, 2026-04-24 갱신)
plan-01 이 4-intent (ANSWER/CLARIFY_REQUEST/GIVE_UP/OFF_TOPIC) 로 확장됐고 OFF_TOPIC 은 L1 에서 직접 분기되므로 근본 원인은 해소. 다만 L1 분류기가 OFF_TOPIC 을 놓친 False Negative (ANSWER 로 샌 OFF_TOPIC/META) 를 L2 에서 재포착하기 위해 가드 로직은 **유지**:
- Step A 가 `claims.length == 0 AND answer_quality <= 1` 이면 `recommended_next_action = "CLARIFICATION"` 강제
- plan-03 Step B 가 이 신호를 받으면 꼬리질문 대신 "질문이 이해되지 않으셨다면 다시 설명드릴게요" 재설명 경로로
- 파급: L1 FN 시 Step A LLM 1회 비용은 지불하되, Step B 꼬리질문 환각은 차단 (defense in depth)
