...
| `backend/src/main/resources/application.yml` | 수정. `rehearse.features.follow-up-pipeline.step-a-enabled` flag |
...
4. p95 latency ≤ 1.5s
5. `step-a-enabled: false` 시 기존 단일 호출 경로로 폴백 (회귀 없음)
6. plan-03(Step B)와 JSON 계약 호환 통합 테스트
7. `progress.md` 02 → Completed
