...
### 분기 라우팅 (의사 코드)
```java
// STT/L3 가 단일 호출(generateFollowUpWithAudio)에 묶여 있어 분리 비용이 큼.
// 따라서 L1 분류는 L3 generation 결과(answerText) 를 입력받아 사후 분기로 동작.
// IntentResponseHandler 인터페이스 + Map<IntentType, IntentResponseHandler> 빈 자동 주입 으로 dispatch.
GeneratedFollowUp followUp = aiClient.generateFollowUpWithAudio(audio, req); // STT + L3 동시
if (followUp.isSkipped()) return aiSkipResponse(followUp);

IntentResult intent = intentClassifier.classify(mainQ, followUp.getAnswerText(), recentDialogue);
return handlers.get(intent.type()).handle(input);  // ANSWER 분기는 followUp 그대로 저장·반환
```

#### L1 호출 위치 결정 (2026-04-25)

**Why L3 후 L1 (사후 분기) 인가**:
- STT(answerText 추출) 는 OpenAI 의 `audio-to-chat` 단일 호출(`generateFollowUpWithAudio`) 안에 통합되어 있어 STT 만 별도 호출하려면 OpenAI Audio API 추가 분리 + 재생성 비용 발생 (2x 호출 + 비용 증가).
- L1 분류는 `answerText` 입력이 필수 → STT 결과 없이 분류 불가.
- 따라서 현 구조는 "L3 호출은 항상 발생하되, OFF_TOPIC/CLARIFY/GIVE_UP 분기에서는 L3 결과를 폐기하고 핸들러 응답으로 대체" 형태.

**Trade-off (수용)**:
- ANSWER 외 분기에서도 L3 LLM 1회 호출 비용 발생 → plan-01 의 "OFF_TOPIC LLM 0회" 목표가 **L3 generation 1회 + handler 0회** 로 후퇴.
- 그러나 L3 가 STT 와 묶인 단일 호출이라 OFF_TOPIC bypass 시 절감 가능한 비용은 generation 토큰 (~max_tokens 만큼) 만 해당. STT cost 는 어차피 발생.
- 비용 절감 대신 **톤·UX 개선** (취조 톤 제거, 비답변에 적절한 응답) 이 본 plan 의 1차 Why → 사후 분기로도 Why 충족.

**향후 분리 옵션** (별도 plan):
- STT (Whisper/Gemini) 를 L3 generation 과 분리 → L1 분류 후 ANSWER 일 때만 L3 호출 → 진정한 "OFF_TOPIC LLM 0회". 본 plan 의 범위 밖.
...
rehearse:
  intent-classifier:
    fallback-on-low-confidence: 0.7
```
...
> **현 구현 노트 (2026-04-25)**: STT/L3 가 `generateFollowUpWithAudio` 단일 호출에 묶여 있어 L1 은 L3 후 사후 분기로 동작. SLA 표는 STT/L3 분리가 가능한 미래 구조 기준 목표치이며, 현 단계 실측 SLA 는 §"현 구조 실측 SLA" 표로 별도 표기.
...
#### 분기별 사용자 path SLA (목표 — STT/L3 분리 후)
...
#### 현 구조 실측 SLA (사후 분기 — STT/L3 미분리)

| 분기 | LLM 호출 구성 | 누적 p95 |
|------|-------------|---------|
| **ANSWER** | STT+L3 (audio chat) + L1 + L2 + L3-rerun | TBD (L3 1회 추가) |
| **CLARIFY_REQUEST** | STT+L3 (결과 폐기) + L1 + clarify LLM | ≤ **3500ms** |
| **GIVE_UP** | STT+L3 (결과 폐기) + L1 + giveup LLM | ≤ **3500ms** |
| **OFF_TOPIC** | STT+L3 (결과 폐기) + L1 + 템플릿 조립 | ≤ **2500ms** |

- **Aggregate p95 ≤ 4,000ms** — 사용자가 답변 제출 → 다음 질문 수신까지. ANSWER 분기 상한. 이 값을 초과하는 plan 은 재설계 필요.
- OFF_TOPIC 분기는 미래 목표 ≤ 500ms / 현 구조 ≤ 2500ms (STT+L3 통합 호출 대기 포함).
