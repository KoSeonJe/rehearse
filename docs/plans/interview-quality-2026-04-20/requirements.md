...
| 지표 | 현재 (추정) | 목표 | 측정 방법 |
|------|-----------|------|---------|
| 꼬리질문 품질 | 추상/기계적 | 사용자 답변 claim 에 정확히 꽂힘 | `MANUAL_AB_PROTOCOL.md` 3~5건 수동 diff (before/after ECR 태그) |
| 피드백 품질 | 추상 ("소통이 좋습니다") | 관찰 인용 + 구체 다음 액션 | `MANUAL_AB_PROTOCOL.md` 3~5건 수동 diff |
| Intent Detection | 0% (미구현) | 구현됨 | 수동 비교 (CLARIFY/GIVE_UP 케이스 의도된 분기 동작 확인) |
| 관찰 인용 포함률 (피드백 gap/strength) | ? | 100% | 정규식 assert (RubricScorer 통합 테스트) |
| 카테고리-차원 적합도 (적용 rubric의 `uses_dimensions` 외 차원 강제 채점 없음) | — | 100% | 자동 체크 (plan-08 Rubric Family, `RubricLoaderTest`) |
| 카테고리-루브릭 매핑 정확도 | — | ≥ 98% | `RubricLoaderTest` 전수 |
| 10턴 세션 평균 입력 토큰 | ~100k (누적) | ≤ 8k | APM 토큰 카운터 (plan-00d Micrometer) |
| L1 Prompt 캐시 히트율 | — | ≥ 95% | Provider 메타데이터 (APM) |
| Resume Extraction claim 커버리지 | — | ≥ 90% | 수동 라벨 vs 추출 결과 (이력서 5건 표본) |
...
- **채택** (사용자 요청 "질문 생성 / 꼬리질문 / 피드백 생성 강화" 직결):
  ...
  - `MANUAL_AB_PROTOCOL.md` 수동 3~5건 비교 (ECR 이미지 2개 병렬 기동)
- **제외** (프로젝트 방향과 안 맞음 / MVP 범위 초과):
  - 플래그 기반 runtime A/B + Judge LLM 기반 자동 평가(골든셋 + J1/J2/J3 Judge 3개) → ECR 롤백 + 수동 diff 로 충분 (2026-04-23 결정)
  - plan-10 Eval Harness Lite / plan-12 Feature Flag Cleanup → 삭제 (위 결정 파생)
  - GDPR 스타일 저장 동의 UI 플로우 → 세션 스코프 캐시만
  - Promptfoo + GitHub Actions CI 통합
...
- **Manual A/B Protocol**: 각 plan 머지 전 ECR 이미지 2개(before/after)를 2개 포트에 병렬 기동 → 동일 녹화본 3~5건 투입 → JSON diff 로 회귀 감지 (`MANUAL_AB_PROTOCOL.md`)
...
- 측정 루프: `MANUAL_AB_PROTOCOL.md` — ECR 2개 병렬 + 수동 diff 3~5건 (plan-10 Judge/골든셋 방식 폐기)
...
| W2 | 상태 영속화 설계 + 관측 인프라 + 피드백 마이그레이션 전략 | 00c State Persistence `[parallel]` 00d Observability `[parallel]` 00e Feedback Migration | **Phase 0** |
...
| W7 | 피드백 품질 | 08 Rubric Scorer + 11 Nonverbal Rubric `[parallel]` → 09 Synthesizer | Phase 4 |
...
- **롤백 수단**: runtime feature flag 없이 ECR 이미지 태그 재배포 + 세션 스토어 캐시 퍼지 (2026-04-23 결정)
