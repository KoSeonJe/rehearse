...
**전이 조건**:
- `INTERROGATION → WRAP_UP`: `clockWatcher.remainingMinutes(session) ≤ WRAP_UP_THRESHOLD_MIN` (application.yml 상수, 기본 2분)
...
Feature Flag runtime toggle은 사용하지 않는다. 값 변경은 `application.yml` 수정 + 재배포로 처리.
...
Feature Flag runtime toggle은 사용하지 않는다. Resume Track 진입은 `interview.type == RESUME_BASED` 조건만으로 결정. 기본 활성화 경로로 단일화.
...
9. 수동 비교(MANUAL_AB_PROTOCOL.md): Resume 세션 3~5건에서 꼬리질문 relevance 과반 이상 우세 확인
