...
| `backend/src/main/java/com/rehearse/api/infra/ai/ResilientAiClient.java` | **수정**. 요청 조립 시 ContextBuilder 결과 사용 |
| `backend/src/main/resources/application.yml` | 수정. `rehearse.features.context-engineering.*` flag |
...
4. OpenAI vs Claude 동일 세션에서 출력 유사도(J1 Judge) ≥ 80%
5. Compaction 결과가 핵심 claim(covered_topics) 누락 없는지 5개 세션 수동 리뷰
6. Feature flag off 시 "전체 히스토리 append" 방식으로 폴백
7. `progress.md` 04 → Completed
