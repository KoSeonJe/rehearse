...
- **G3 (evidence_quote source 차이) — 사용자 결정 2026-05-17 정정**: verbal scorer / Gemini audio = transcript substring (사용자 발화) 강제 유지. **vision = frame 자체 인용** (예: "00:45 구간 시선 이탈" / "프레임 중반 자세 흔들림"). vision evidence_quote 가 transcript substring 일 필요 없음 — `dimension_validator` 에 `stage` 파라미터 추가 (`verbal` / `gemini` / `vision`) 후 `stage="vision"` 시 substring 룰 skip. vision_analyzer 의 transcript 입력 폐지 (이전 spec 의 "vision = transcript substring 강제" 결정 반려). 폐기 사유: transcript 가 비어 있거나 evidence_quote 가 영상 신호 묘사일 때 100% 실패 → vision dimension 영구 omit 회귀.
...
- **`backend/src/main/java/com/rehearse/api/infra/ai/adapter/RubricScorerResponseValidator.java` (신규)** — 정책 단일화. score / observation 한국어 음절 / evidence_quote substring 검증 + 위배 사유 enum (`InvalidScore`, `MissingObservation`, `NonKoreanObservation`, `MissingEvidence`, `EvidenceNotInUserAnswer`) 반환. **Phase 1 verbal + Phase 2a 비언어 LLM 호출 시 본 클래스 재사용 (BE 단 단일 출처)**. 위치 = `infra/ai/adapter/` (도메인 무관 정책).
  - **Lambda 대칭 (P1-D 정합)**: `lambda/analysis/analyzers/dimension_validator.py` (신규, 1 모듈) — gemini / vision 분석기 공유. 동일 정책 (한국어 음절 / score 1-3). evidence_quote source 는 `stage` 파라미터로 분기 — `stage="verbal" | "gemini"` = transcript substring 강제, `stage="vision"` = 프레임 자체 인용 허용 (substring 검증 스킵). Java validator = verbal scorer 전용 → stage 개념 부재 (단일 분기). enum naming drift 허용 (Java `EVIDENCE_NOT_IN_USER_ANSWER` vs Python `EVIDENCE_NOT_IN_TRANSCRIPT` — 같은 의미, 호출 컨텍스트 차이).
  - **정책 동기화 절차 (P1-D)**: `RubricScorerResponseValidator.java` + `dimension_validator.py` = **동일 PR 변경 강제** (review checklist 항목: "validator 정책 변경 시 BE / Lambda 양쪽 동시 갱신 여부 확인"). 정책 명세 단일 출처 = `_dimensions.yaml` 의 `score` 정의 (1-3) + 본 tech-spec 의 transcript substring / 한국어 음절 / vision frame 인용 룰. 정책 drift 방지 = BE / Lambda validator 단위 테스트 동일 케이스 fixture 공유 (한국어 음절 매치 / 자모 단독 / evidenceQuote substring 매치 / whitespace 정규화 = 동일 입출력 표). vision stage 케이스는 Python 단독 (Java 대응 부재).
...
| 동시성 | 단일 turn 처리 컨텍스트 내부. 동시 turn racing 영향 없음. NonverbalScorePersister listener async = 트랜잭션 내 `saveRubric("nonverbal-v1", ...)` 호출 단일. Lambda ThreadPoolExecutor turn 병렬 실행 시 `dimension_validator.validate(...)` = stateless 순수 함수 (instance 상태 부재 / 모듈 변수 mutation 부재) → race condition 부재 | listener async 격리 + dimension_validator 순수 함수 | 확신 |
