...
**설계 결정 (가이드 톤 채택)**: OPENER 시점 LLM 입력 = projectName / 역할 / 기술 키워드 (이력서 메타). 응시자 본인 경험 디테일 (Situation / Action / Result 의 실제 수치) **모름**. 1인칭 답변 예시 강제 시 LLM 이 상상한 placeholder 출력 → 거짓 학습 자료 / 응시자 혼란. 따라서 **답변 구조 권유 가이드 톤** 채택. INTERROGATION 과 분기 사유 = chain 내부에서는 직전 응시자 발화 + topic 누적이 LLM 에 전달되어 구체 1인칭 placeholder 생성 가능.

**1-1. model_answer 정의 갱신** (line 17):

- 변경: 200~300자 / 3~5줄 / 답변 구조 권유 가이드 톤 ("~ 구조가 효과적입니다", "~을 정리하세요", "~ 마무리하면", "~을 짚어주세요") / projectName 명시 / STAR·결정 근거·트레이드오프·학습 중 1+ 구조 단서 포함. 1인칭 답변 예시 ("저는 ~ 했습니다") **금지** — 거짓 placeholder 위험.

**1-2. 자가검증 라인 갱신** (line 42):

- 가이드 톤 어구 1+ 매치 (효과적입니다 / 구조가 / 마무리하면 / 정리하세요 / 짚어주세요 / 서술하세요). 1인칭 답변 예시 어미 발견 시 폐기 후 재작성.
- projectName 명시 + 지시 표현 단독 사용 금지.
- 구조 단서 1+ 포함.

**1-3. safe-fallback question 텍스트** (line 45): 변경 없음.

### 변경 2: `resume-playground-responder.txt`

**설계 결정**: Responder 시점 LLM 입력에 USER_ANSWER 핵심 명사 포함됨. 그러나 응시자 답변 = 단편적 (Situation 정도만 노출. Action / Result 디테일 미포함 케이스 다수). 1인칭 placeholder 강제 시 LLM 이 상상한 디테일 추가 → 거짓 학습. 따라서 OPENER 와 동일하게 **가이드 톤** 채택. 단 USER_ANSWER 핵심 명사 1+ 재인용은 강제 (사용자 맥락 반영).

**2-1. model_answer 정의 갱신** (line 30):

- 변경 1-1 동일 가이드 톤 정의 + Responder 1턴 맥락 (직전 발화 핵심 명사 1+ 재인용 강제) 추가.

**2-2. 자가검증 라인 갱신** (line 69):

- 변경 1-2 동일 가이드 톤 가드 + USER_ANSWER 핵심 명사 1+ 재인용 라인 유지.
