# Plan 01: Flyway 도입 + 초기 마이그레이션 + dev-server 프로필
...
| `backend/src/main/resources/application-dev-server.yml` | 개발 서버 프로필 |

## 상세

- Flyway 의존성: `flyway-core` + `flyway-mysql`
- V1 마이그레이션: ERD 기준 9개 테이블 + FK + 인덱스
- dev-server 프로필: MySQL + validate + Flyway enabled + baseline-on-migrate
