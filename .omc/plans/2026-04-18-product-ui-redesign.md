...
- **Tier 2**: 12-col asymmetric grid + editorial 3-pane + 6개 structural primitive 신설
...
| Pretendard 본문 + Cal Sans display | ✓ 확정 | **Cal Sans 유지**, Fraunces는 editorial 문맥만 optional 추가 |
...
| editorial 타이포 | ✗ | **신규 추가** (Fraunces optional) |
...
--accent-editorial: 33 75% 40%;          /* #b8741a */
--accent-editorial-bg: 40 80% 95%;       /* #fdf4e7 — 배경 tint */
...
- `--accent-editorial`과 `--signal-warning`은 같은 viewport에 등장 가능하나, 인접 요소에 동시 적용 금지 (Ochre vs Amber 혼동 방지, 8px 이상 간격 유지)
...
**기본 원칙**:
- Pretendard 본문 필수 (승계, Plan 01~09 합의)
- **Cal Sans Display 유지** — 기존 DESIGN.md·Plan 01~09 정책 승계. 영문 display의 기본 폰트
- Fraunces serif는 **Optional 신규 추가** — Cal Sans를 대체하지 않고, editorial 문맥(ChapterMarker 숫자, 피드백 질문 제목)에만 선택 적용
- 한국어 헤딩은 Pretendard 굵기·자간 변주로 위계 구성

> **trade-off 근거**: Cal Sans는 이미 Plan 01~09에서 확립된 display 폰트. 제거 시 리그레션 위험. Fraunces는 editorial 강조가 필요한 신규 컨텍스트(챕터 마커 등)에만 얹고, 한국어 글자에는 적용되지 않으므로(폰트 믹스 회피) 비용(약 80KB) 대비 정당화된다.

#### Display 폰트 사용 규칙
- **Cal Sans**: 기본 display. 영문 히어로 타이틀, 대형 헤드라인 등 Plan 01~09가 정한 영역에 계속 사용
- **Fraunces**: editorial 전용. ChapterMarker `01` 숫자, 피드백 질문 제목 (Priority 1 페이지만). 다른 페이지 신규 도입 금지
- 두 폰트를 **같은 섹션에 함께 배치 금지** — 시각 충돌 방지
...
토큰 정의만으로는 부족하다. 아래 3가지 용례를 기준 패턴으로 사용한다:
...
export const TabBar = ({ items, activeId, onSelect, className }: TabBarProps) => (
  <nav
    className={cn(
      ...
      'bg-background/95 backdrop-blur-sm border-b border-foreground/8',
      'px-4 gap-1',
      className,
    )}
    aria-label="질문 탭"
...
| Cal Sans display | ✓ | **유지** (영문 display 기본). Plan 01~09 연속성 존중 | 승계 | 유지 |
| Fraunces serif (신규) | ✗ | editorial 문맥(ChapterMarker, 피드백 질문 제목)에만 optional 추가 | 신규 추가 | §3 Typography에 Fraunces subsection append |
