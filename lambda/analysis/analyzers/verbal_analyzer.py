...
5. 답변 구조 (structure_level)
   - GOOD: 서론-본론-결론 등 명확한 흐름
   - AVERAGE: 대체로 흐름 있으나 일부 산만
   - NEEDS_IMPROVEMENT: 두서없이 나열
6. 구조 피드백 (structure_comment): 구조적 관점 1-2문장
7. 태도 인상 (attitude_comment): 면접관 관점 태도/말투 인상. ✓ 긍정 / △ 부정 / → 개선
...
반드시 아래 JSON 형식으로만 응답하세요:
{
  ...
  "comment": "✓ 잘한 점\n△ 보완할 점\n→ 개선 방법",
  "structure_level": "<GOOD|AVERAGE|NEEDS_IMPROVEMENT>",
  "structure_comment": "<구조적 관점 1-2문장>",
  "attitude_comment": "✓ 긍정\n△ 부정\n→ 개선"
}"""
...
def analyze_verbal(
    ...
    if not transcript or not transcript.strip():
        ...
        return {
            ...
            "structure_comment": None,
            "attitude_comment": None,
        }

    client = OpenAI(api_key=Config.OPENAI_API_KEY)

    if position:
        system_prompt = build_system_prompt(position, tech_stack, feedback_perspective)
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            print(f"[Verbal] 분석 완료: fillers={result.get('filler_word_count')}")
            return result
